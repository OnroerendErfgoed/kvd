...
class VIOE_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected $tags = array(
                       ...
                       'see'        => array(
                                        'required'       => false,
                                        'allow_multiple' => true,
                                        'order_text'     => 'follows @link (if used) or @license',
                                       ),
                       'since'      => array(
                                        'required'       => false,
                                        'allow_multiple' => false,
                                        'order_text'     => 'follows @see (if used) or @link (if used) or @license',
                                       ),
                       'deprecated' => array(
                                        'required'       => false,
                                        'allow_multiple' => false,
                                        'order_text'     => 'follows @since (if used) or @see (if used) or @link (if used) or @license',
                                       ),
                );
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$commentStart]['code'] === T_CLOSE_TAG) {
            ...
            if ( $short == '' ) {
                $error = "A file level docblock must not have a short description.";
                $phpcsFile->addError($error, ($commentStart + 1));
            }

            $long        = $comment->getLongComment();
            if ( empty($long) !== true ) {
                $error = "A file level docblock must not have a long description.";
                $phpcsFile->addError($error, ($commentStart + 1));
            }
        ...
        if ($package !== null) {
            ...
            if ($content !== '') {
                if ( !(strtolower( $content ) === $content) ) {
                    ...
                    $validName = trim(strtolower($newContent));
    ...
    protected function processVersion($errorPos)
    {
        ...
        if ($version !== null) {
            ...
            if (empty($content) === true) {
                ...
                $error = "Invalid version \"$content\" in file comment; consider \"" . '$Id$" instead';
                $this->currentFile->addWarning($error, $errorPos);
            }
        }
