...
class KVDthes_SqlGenerator
{
     ...
     * @return void
     */
    public function __construct( KVDthes_ISessie $sessie )
    {
        $this->sessie = $sessie;
    }
     ...
     * generateSql 
     * 
     * @param string    $mapperName         Naam van de mapper die de thesaurus kan leveren. Normaal is dit een xml-mapper.
     *                                      Eventueel kan dit op termijn een ander soort mapper zijn zoals een csv-mapper.
     ...
     */
    public function generateSql( $mapperName, $thesaurus_id, $thesaurus_naam )
    {
        $sql = "--Thesaurus.\n";
        ...
        $sql .= "--Termen.\n";
        $termen = $this->sessie->getMapper( $mapperName )->findAll( );
        foreach ( $termen as $term ) {
            $sql .= sprintf( "INSERT INTO thes.term VALUES ( %d, %d, '%s', '%s', '%s');\n", $thesaurus_id, $term->getId( ),$term->getTerm( ), $term->getQualifier( ), $term->getLanguage() );
        }
        $sql .= "\n";
        ...
        $sql .= "--Visitation.\n";
        $topTerm = $this->sessie->getMapper( $rd->getParameter( 'class') )->findRoot( );
